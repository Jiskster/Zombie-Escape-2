...
return "Intermission", function(v, player)
	...
	local wtics = game.win_tics
	local team_won = game.team_won
	...
	if game.round == game.maxrounds then
		local diff_a = min(max(0, game.win_tics - newroundframe), slideout_anim)
		local div_a = FixedDiv(diff_a*FU, slideout_anim*FU)

		if game.win_tics >= newroundframe then
	...
	if game.win_tics >= newroundframe + slideout_anim
	and not vote.nextmapvoted and game.round == game.maxrounds then
		local slideintics = game.win_tics - (newroundframe + slideout_anim)
	...
	elseif vote.nextmapvoted then
		local x = 160*FU
		local y = 100*FU
		local newmap = vote.nextmapvoted
