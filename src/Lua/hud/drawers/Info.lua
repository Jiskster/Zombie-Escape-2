...
local function health(v,p,me,ze)
	...
	if drawshields
		...
		local x = z
		local y = y - ((ze.team == 1) and ((height+pad)*2) or (height + pad))
		if store_info.shield ~= -1
			local diff = max(abs(store_info.shield - shields)/2, 1)
			if diff > 0
				local shake = (diff*FU)/4 * (leveltime & 1 and 1 or -1)
				y = $ + shake
				x = $ + shake
		...
		local width = FixedMul(max_width, FixedDiv(shields,maxshields))

		drawSkewFill(v, x+shadow,y+shadow, max_width,height, flags, SKINCOLOR__31) --31
		if old_info.shield > shields
			if store_info.shield == -1
				store_info.shield = old_info.shield
			end
			local s_width = FixedMul(max_width, FixedDiv(store_info.shield,maxshields))
			drawSkewFill(v, x,y, s_width,height, flags|V_20TRANS, SKINCOLOR__35) --35
		else
			if store_info.shield ~= -1
			and (leveltime % 3 == 0)
				local diff = 1 --max(abs(store_info.shield - shields)/5, 1)
				if store_info.shield < shields
					store_info.shield = $ + diff
				else
					store_info.shield = $ - diff
				end
			end
			if store_info.shield == shields
			or store_info.shield == -1
				store_info.shield = -1
			else
				local s_width = FixedMul(max_width, FixedDiv(store_info.shield,maxshields))
				drawSkewFill(v, x,y, s_width,height, flags|V_20TRANS, SKINCOLOR__35) -- 35
		...
		drawSkewFill(v, x,y, width,height, flags, color)
		v.drawString(x + textspace, y + shadow,
			string.format("%d | %d", shields, maxshields),
			flags, "thin-fixed"
		)
		if shields <= 0
		and store_info.shield == -1
