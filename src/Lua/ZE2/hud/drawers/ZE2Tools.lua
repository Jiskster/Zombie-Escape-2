...
local Tools_HUD = function(v, p)
	...
	if #ZE2.Checkpoints then
		drawString(tp_x, tp_y, "\130Teleport to checkpoints with:", tp_flags, "thin")
		drawString(tp_x, tp_y+spacing, "zd_nextcheckpoint", tp_flags, "thin")
		drawString(tp_x, tp_y+(spacing*2), "zd_prevcheckpoint", tp_flags, "thin")
	else
		drawString(tp_x, tp_y, "No checkpoints to teleport", tp_flags|V_REDMAP, "thin")
	end
end
